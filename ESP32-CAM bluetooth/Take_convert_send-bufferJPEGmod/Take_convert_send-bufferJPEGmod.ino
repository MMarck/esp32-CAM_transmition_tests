...
void callback(esp_spp_cb_event_t event, esp_spp_cb_param_t *param){
  ...
  if(event == ESP_SPP_DATA_IND_EVT){
    ...
    if( option == 'x' )
    {
      ...
      uint8_t quality = 100; // value between 1 to 100
      if(!fmt2jpg(pic->buf, pic->len, pic->width, pic->height, pic->format, quality,  &cnv_buf, &cnv_buf_len))
